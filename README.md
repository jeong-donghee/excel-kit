...
**목록 → 엑셀 다운로드** ([`excel-kit-grid`](excel-kit-grid))
```java
...
**차트 → 엑셀** ([`excel-kit-canvas`](excel-kit-canvas))
```java
@ExcelChart(type = ChartType.BAR, title = "월별 매출")
class Sales { @ChartCategory String month; @ChartSeries(name = "매출") int revenue; }

ExcelCanvas.create().chart(salesList).writeTo(Path.of("dashboard.xlsx"));
```

각 모듈 README에 자세한 사용법·설치가 있다.
